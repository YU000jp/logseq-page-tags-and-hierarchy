...
export const removeHierarchyPageTitle = async (lastPart: string, fullName: string) => {
    // 日誌は除外
    const pageTitleElement = parent.document.body.querySelector("div#root>div>main div#main-content-container div.page.relative:not(.is-journals) div.ls-page-title h1.page-title span.title") as HTMLDivElement | null
    if (!pageTitleElement
